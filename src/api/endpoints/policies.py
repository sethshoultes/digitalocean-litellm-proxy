...
from src.schemas.policies import (PolicyCreate, PolicyResponse, PolicyUpdate,
                                  UserPolicyAssignment, UserPolicyResponse)
...
async def create_policy(
    ...
    try:
        ...
        existing_query = select(AccessPolicy).where(
            AccessPolicy.policy_name == policy.policy_name
        )
...
async def get_policy(
    ...
    try:
        ...
        if (
            current_user.user_role not in ["PROXY_ADMIN", "ORG_ADMIN"]
            and policy.is_system_policy
        ):
            raise HTTPException(
...
async def update_policy(
    ...
    try:
        ...
        if (
            policy_update.policy_name
            and policy_update.policy_name != policy.policy_name
        ):
            existing_query = select(AccessPolicy).where(
...
async def assign_policy_to_user(
    ...
    try:
        ...
        assignment_query = (
            select(UserAccessPolicy)
            .options(selectinload(UserAccessPolicy.policy))
            .where(
                and_(
                    UserAccessPolicy.user_id == assignment.user_id,
                    UserAccessPolicy.policy_id == policy_id,
