...
from .base import (Base, ConnectionStatusType, ProviderTypeEnum, TimestampMixin)
...
class UserConnection(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(
        ConnectionStatusType, default="active", index=True
    )
    ...
    health_status: Mapped[Optional[str]] = mapped_column(Text, default="unknown")
    error_count: Mapped[int] = mapped_column(Integer, default=0)
    success_count: Mapped[int] = mapped_column(Integer, default=0)
    avg_response_time: Mapped[Optional[float]] = mapped_column(Numeric(10, 3))
    ...
    shared_connections: Mapped[List["SharedConnection"]] = relationship(
        "SharedConnection", back_populates="connection", cascade="all, delete-orphan"
    )
    activities: Mapped[List["ConnectionActivity"]] = relationship(
        "ConnectionActivity", back_populates="connection", cascade="all, delete-orphan"
    )

    __table_args__ = (
        CheckConstraint(
            "char_length(connection_name) >= 3", name="valid_connection_name"
        ),
    )
...
class ConnectionTemplate(Base, TimestampMixin):
    ...
    provider: Mapped[str] = mapped_column(ProviderTypeEnum, nullable=False, index=True)
    configuration_template: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False
    )
    required_fields: Mapped[List[str]] = mapped_column(ARRAY(Text), nullable=False)
    optional_fields: Mapped[List[str]] = mapped_column(ARRAY(Text), default=list)
...
class SharedConnection(Base):
    ...
    shared_with_id: Mapped[str] = mapped_column(Text, nullable=False)
    permissions: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False, default=lambda: {"read": True, "use": True}
    )
    shared_by: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    connection: Mapped["UserConnection"] = relationship(
        "UserConnection", back_populates="shared_connections"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "expires_at IS NULL OR expires_at > shared_at", name="valid_share_expiry"
        ),
    )